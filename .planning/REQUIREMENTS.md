...
- [ ] **DSGN-01**: Each scouted friction claim is verified against the codebase with a recorded verdict (confirmed / false / partial), file:line evidence, and an "already exists vs. needs building" note — so we expose/document rather than rebuild.
- [ ] **DSGN-02**: A single reviewed API-design doc specifies the new introspection / test-ergonomics / devtools surface (names, signatures, return shapes, serialization, ownership across engine/session/runtime/ui) and is approved before implementation begins.
