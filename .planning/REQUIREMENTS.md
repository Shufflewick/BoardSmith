...
- [x] **INGEST-01**: Designer can ingest a rulebook (PDF/images/text) that is transcribed once by fan-out subagents into canonical `rulebook/` slices with citations, each confirmed with the user
...
- [x] **INGEST-03**: Designer with no written rulebook can use an interview fallback that produces the same `rulebook/` files section by section
- [x] **INGEST-04**: Ingest scaffolds the project (`boardsmith init`, naming rules) and verifies the skeleton compiles and serves before rules work begins
- [x] **INGEST-05**: Ingest proposes a SKETCH.md whose first chunk is the core event loop, that mandates game-end and final-acceptance chunks, tags chunks `ui: none|touches|major`, uses outcome-based test scripts, and gates on user approval with chunk-count/time expectations
...
| INGEST-01 | Phase 142 | Complete |
| INGEST-02 | Phase 142 | Complete |
| INGEST-03 | Phase 142 | Complete |
| INGEST-04 | Phase 142 | Complete |
