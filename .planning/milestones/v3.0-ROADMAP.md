...
**Success Criteria** (all TRUE):
  1. Animation events arriving from the server are queued FIFO and processed in order
  2. When the queue encounters an event type with no registered handler, processing pauses (up to a configurable timeout, default 3s)
  ...
  5. `skipAll()` clears the queue instantly, and `isAnimating` / `pendingCount` remain reactive throughout
...
**Success Criteria** (all TRUE):
  ...
  5. cribbage game animations work correctly with the new system (verified in browser)
...
- Playback 100% client-owned (server never waits on animation, no per-player tracking)
- Animation-driven components render from event data, not state (eliminates state/animation sync bugs)
- Events on command stack (future replay/rewind capability)
...
- handlerWaitTimeout supersedes defaultDuration for no-handler case

**Issues Resolved:**

- Theatre view chicken-and-egg problem with component mounting (CombatPanel opens late / never)
- Mutation capture fragility for complex state
- Server-side acknowledgment protocol creating unnecessary complexity
- Theatre snapshot shared across players breaking multiplayer
- Cribbage animation events reading from wrong path (fixed during audit)
- Orphaned acknowledgeAnimationEvents dead code (removed post-audit)
...
- None (orphaned acknowledgeAnimationEvents was fixed pre-milestone completion)
