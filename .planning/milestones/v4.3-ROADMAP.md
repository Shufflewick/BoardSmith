# Milestone v4.3: Agent-Ready Engine — Introspection, Test Ergonomics & Devtools

**Status:** ✅ SHIPPED 2026-07-01
**Phases:** 116–122 (7 phases)
**Total Plans:** 20 plans + 4 documentation deliverables
**Git range:** v4.2..v4.3 (126 commits)

## Overview

Made BoardSmith drivable by agents (and humans) end-to-end without vision or coordinate-clicking. Shipped a single serializable action-space introspection primitive, ergonomic test APIs that explain their own failures, a dev-host devtools bridge with stable selectors + an observable action-resolved signal, fail-fast pit-of-success authoring guards, migrated every example game + the MERC canary onto the surface, and documented the whole thing. Reuse-not-rebuild throughout: where an API already existed it was exposed/documented rather than duplicated.

## Key Accomplishments

1. **Action-space introspection (117):** one serializable `getActionSpace(seat)` answering "what can this seat do now, with what choices?" + `getActionSchema`, `buildActionArgs`, `enumerateLegalMoves`, and a typed perspective-aware `getPlayerView`. The MCTS bot now delegates to the shared `enumerateSelectionsCore`.
2. **Test ergonomics (118):** typed observable state, one-call `playUntilComplete` with structured `GameStuckError`, self-explaining assertion traces, explicit exact/contains action assertions, and a multi-step `ActionBuilder` — plus 3 simultaneous-turn bugs found and fixed in code review.
3. **Dev-host devtools bridge (119):** stable `data-bs-el-id` selectors in custom UI AND AutoUI, a read-only `window.__BOARDSMITH_DEVTOOLS`, and a `boardsmith:action-resolved` signal — composing into a vision-free, poll-free DISCOVER→SELECT→DRIVE→CONFIRM loop, live browser-proven.
4. **Authoring pit-of-success guards (120):** `loop()` without `maxIterations` now throws at construction; game start validates element registration and action reachability; two new ESLint rules (`no-element-identity-comparison`, `no-element-array-state`).
5. **Migration + guard hardening (121):** all 7 example game suites, BoardSmith (1873 tests), and the re-vendored MERC canary (738 tests) green; migration surfaced and fixed PIT-02 falsely firing on framework built-ins (`DicePool.all(Die)`) via a built-in-class exemption where the actually-instantiated class is authoritative.
6. **Documentation (122):** new agent-control + browser-testing guides, updated testing + common-pitfalls docs — every snippet verified against `src/`, with several stale API references removed.
