...
- [x] **ENG-03**: A failed action inside `simultaneousActionStep` surfaces `actionError`, returns failure to the client, and is NOT recorded in `actionHistory` (F5)
...
- [x] **SDK-03**: `MeepleClient` methods have ONE consistent error contract (all throw or all return results) — no silent raw-JSON failure half (F25)
...
- [x] **SDK-05**: `WebSocketMessage` union includes all message types actually sent (adds `UpdateSlotPlayerOptionsMessage`) (F35)
- [x] **SDK-06**: The playerId error message points at a field that exists (message or config type fixed) (F38)
...
- [x] **TST-02**: `TestGame`'s default seed is deterministic (fixed literal, not `Date.now()`) (F37)

### Documentation Corrections (DOCX)

- [x] **DOCX-01**: docs/core-concepts.md no longer teaches the removed event-sourcing command model or nonexistent `element.setAttribute()` (F11)
- [x] **DOCX-02**: `registerActions()` JSDoc models the real API (F14)
- [x] **DOCX-03**: docs/getting-started.md documents the CLI that actually exists (F20)
...
- [x] **GAMES-02**: MERC re-vendored onto the new version and green; gaps surfaced during migration fixed in BoardSmith `src/`, not worked around
...
| GAMES-02 | Phase 138 | Complete |
| DOCX-01 | Phase 139 | Complete |
| DOCX-02 | Phase 139 | Complete |
| DOCX-03 | Phase 139 | Complete |
