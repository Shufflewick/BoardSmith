# Milestone v4.5: Pit of Success Hardening (Audit #3 Fixes) — SHIPPED 2026-07-03

**Phases:** 131–139 (9/9 complete) · **Requirements:** 42/42 · **Audit:** passed (`.planning/v4.5-MILESTONE-AUDIT.md`)

**Stats:** 264 commits (2026-07-02 → 2026-07-03) · 106 source files changed, +8,897/−1,032 LOC (excl. .planning) · BoardSmith suite 2081 → 2371 tests (175 files) · 8 games + MERC (738) migrated & green

**Key accomplishments:**
1. Killed the critical hidden-info leak: zone visibility now serializes and survives every restore path (undo, rewind, restore, stateless ops) — byte-identical opponent views, browser-verified.
2. Implemented `visibleAttributes` + per-viewer `state.players` filtering, debug-broadcast opt-in, and teachingDisabled persistence — the whole restore/visibility cluster fixed at the serialization layer.
3. Engine fail-loud sweep: putInto cycle throw, resolveArgs no bare-number coercion, forEach snapshots, handler-less action rejection, eachPlayer wrap-around, simultaneous actionError + FlowHaltedError contract, server-side multiSelect enforcement, switchOn throw.
4. UI/session guardrails: single GameShell toast chokepoint (errorTick), read-only session.runner facade, fill()/dragProps/hook contracts fixed; CLI hardened (localhost default, fail-fast flags, schema-validated boardsmith.json, one player-count source of truth, 50MB real limit).
5. Client SDK rebuilt around an awaitable connection lifecycle (opened/awaitReconnect), one throwing error contract (MeepleClientError), canonical protocol types; TestGame now deterministic + fail-loud (doAction throws, 'test-seed' default).
6. Verify-first discipline throughout: 34 findings re-proven before fixing; adversarial review loops caught ~11 critical + ~34 warning defects post-execution; --no-open dev-host seat-race found & fixed via headless Playwright smokes.
