...
| GEN-01 | Phase 150 | Complete (reopened follow-up) |
| PLAY-01 | Phase 151 | Complete (reopened follow-up) |

**Coverage:** 36/36 v4.6 requirements mapped (34 original + GEN-01/PLAY-01 from the reopened playtest follow-up), no orphans, no duplicates.

## Reopened Playtest Follow-Up (Phases 150–151)

v4.6 shipped 2026-07-05 with VAL-01's human browser-playtest deferred (the Phase-149 dry-run built its game into a throwaway `/tmp` dir that cleanup deleted). Reopened to close the gate honestly:

- [x] **GEN-01** (Phase 150): the `bs-` pipeline regenerated the Go Fish chunk-1 game into a durable, non-throwaway location (`~/BoardSmithGames/go-fish-dryrun/`) that compiles + serves + passes its full automated bar.
- [x] **PLAY-01** (Phase 151): a human playtested the pipeline-built game, closing VAL-01's deferred gate. The gate caught a real pre-existing dev-host lost-update race (DEF-B, fixed at source, BoardSmith `281e8155`) + a missing-card-assets defect (DEF-A, fixed) — issues no automated check surfaced.
