# v4.6 BS Skills (Rulebook-Driven Game Building) — SHIPPED 2026-07-05

Status: complete · 12 phases (140-149 + reopened 150-151) · 36/36 requirements · audit passed (`v4.6-MILESTONE-AUDIT.md`) · human playtest gate CLOSED

## Playtest Follow-Up (Phases 150–151, reopened + closed 2026-07-05)

The original ship deferred VAL-01's human browser-playtest (the Phase-149 dry-run built its game into a throwaway `/tmp` dir that cleanup deleted). v4.6 was reopened for two phases to close it honestly:

- **Phase 150 — Regenerate the pipeline-built Go Fish (stable location) [GEN-01]:** re-ran the `bs-` ingest + chunk-1 build legs into a durable `~/BoardSmithGames/go-fish-dryrun/` (compiles, 38/38 tests, random-sim to terminal, dual hidden-info leak proofs, serves; go-fish/ read-only). Passed.
- **Phase 151 — Human playtest [PLAY-01]:** the user playtested the pipeline-built game. The gate caught **DEF-A** (missing card assets — fixed) and **DEF-B** (a real pre-existing dev-host lost-update race — fixed at source, BoardSmith `281e8155`, per-host op-queue serialization, full suite 2653/2653) plus **DEF-C** (a 2-browser turn-desync, investigated, not reproducible post-fix, watch item). Passed — VAL-01's gate is closed. The human gate earned its keep by surfacing a concurrency bug every automated check shipped green.

Follow-ups tracked as tech debt: `bs-build-chunk` asset-completeness skill gap; DEF-C reconnect watch item; MERC re-vendor for the DEF-B fix.
