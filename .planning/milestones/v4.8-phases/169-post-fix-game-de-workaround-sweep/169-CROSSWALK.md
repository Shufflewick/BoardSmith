...
| BS-3 | GameShell board zoom computed once at mount, never re-fits on resize | D12 | ZOOM-01 | 158 | `src/ui/board-height.ts` + `GameTable.vue:55-68` board-height cap | **169-05 OUTCOME: KEPT-AND-NOTED, NOT removed.** D12/ZOOM-01 confirmed PRESENT (`ResizeObserver`-driven re-fit in `useAutoZoom.ts`). Assessed whether the board-height cap (`board-height.ts`) is now a pure stale-fit compensator: it is NOT — it caps the board's own height so the fit's height ratio is always ≥1 (zoom=1 by construction), enforcing DESIGN.md's 220px card-legibility floor independent of whether the fit re-fits dynamically on resize. Even with ZOOM-01's re-fit, a board taller than its region would still shrink below the floor without this cap. Kept; not a BS-3/D12 workaround, a design-floor mechanism. |
...
| BS-5 | `chooseElements` (multi-element pick) unusable from UI — click path no-ops, auto-select throws | D9 | AI-01 | 159 | `src/rules/actions.ts:39-86` + `roll-conditions.ts:273-276` + `App.vue:14-17` single-select enumeration workaround | **169-05 OUTCOME: DEFERRED, comment-only.** D9/AI-01 confirmed PRESENT. The native `chooseElements`+function-`multiSelect` rewrite was assessed and explicitly deferred — it's a risky change to the game's central interaction (4 files, board click-driver included), not a bug fix; the current single-select-of-satisfying-sets encoding already works end-to-end. Stale BS-5 comments in `actions.ts`/`roll-conditions.ts`/`App.vue` refreshed to record D9/AI-01 closed the original panel gap (defect 1) while the rewrite stays deferred. BS-5's defect 2 (auto-fill scalar `TypeError`) is untouched by D9 and remains open; doom's `optional:true` opt-out stands. Suite unchanged (399/405; 6 pre-existing unrelated failures, see `deferred-items.md`). |
...
| BS-9 | a `Space` can never be removed from the element tree | D23 | SPACE-02 | 163 | `src/rules/elements.ts:92` + `game.ts:1223` one-slot-one-pile workaround | **169-05 OUTCOME: KEPT-AND-NOTED, NOT removed.** D23/SPACE-02 confirmed PRESENT (`Space.remove()`/`reparent()` via `moveToInternal`, `src/engine/element/space.ts:127-159`) — BS-9's originally-reported API gap is closed upstream. The `MachineRow` spare-slot/reseat model was assessed for teardown but is NOT purely a compensating workaround: it's Decision 18 ("`slotIndex` is a POSITION, never an identity") + Decision 31 baked together, load-bearing for `reseat()`/`slideRight()`'s idempotent renumbering and several rulebook adjacency-read effects. Swapping to destroy-and-recreate via the now-available API would be an architectural change to the row's data model — out of scope for this conservative, extra-cautious single-game sweep. Kept; noted for a future BS-9-specific plan. |
| BS-10 | dev host serves game iframe at sub-path with no `<base>`, relative `public/` assets 404 | — (not in D1-D32 battery; dev-host defect) | n/a | n/a | n/a | **169-05 OUTCOME: RECLASSIFIED, not an open library bug.** Verified by inspection: `src/rules/cards.ts` (20 refs) + `DoomMachineTracker.vue`/`PlayerHpTracker.vue` already use absolute `/cards/*.png` paths (commit `6949fde`, 2026-07-20) — a game-side art-path fix already handled, predating this sweep. `BOARDSMITH-BUGS.md` BS-10 updated to record the reclassification. The dev host's `<base href="/">` gap is still real but doom no longer depends on it; it is folded into a **scaffold-default recommendation** below (not an engine change, not re-fixed here). |
...
| Item | Status | Note |
|---|---|---|
| D32 / `[DRAWDROP]` | no-op | Platform-side (deployed web front-end + Convex `pieces:*` mutations), proven ABSENT from this library repo and all 5 game repos per Phase 165. Nothing to remove in any game repo. **169-05 re-confirmed:** `grep -rn "DRAWDROP" src/` in doom-machine returns nothing. |
| BS-10 `<base href="/">` scaffold gap | **RECOMMENDATION (not implemented here)** | 169-05: doom-machine's dev-host iframe (`/__boardsmith/play`) serves no `<base href="/">`, so a relative `public/` asset path (`cards/x.png`, the old scaffold's idiom) 404s to the SPA fallback under the sub-path route. Doom worked around this at the GAME level (absolute `/cards/x.png` paths, commit `6949fde`) rather than the library fixing the serve. **Recommendation for a future scaffold/docs plan:** the `boardsmith` project scaffold (and any example/doc snippets referencing `public/` assets) should model ROOT-RELATIVE (`/cards/x.png`) paths from the start — the Vite `public/` convention doom's fix now follows — rather than the relative form the current scaffold produces, which breaks silently the moment a game is served at a sub-path. This is a documentation/scaffold-default change, not an engine change, and is explicitly NOT implemented by 169-05 (out of scope per CONTEXT decisions — reclassify, don't re-fix). |
