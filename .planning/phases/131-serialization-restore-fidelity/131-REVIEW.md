...
**Resolution:** status: fixed — `executeOp` gained a dedicated 6th `hostOptions?: { teachingDisabled?: boolean }` parameter (threaded positionally like `pendingState`); hint/heatmapToggle/startTutorial guards now read it instead of `gameOptions`. `MultiplayerHost` removed `teachingDisabled` from both `startGameOptions` and `baseOptions` and passes it via `hostOptions` (so it no longer persists into `snapshot.gameOptions`); `dev.ts` `RuntimeExecuteOp` and all test harness lambdas forward the new parameter. New tests: D-01 non-collision (a game-defined `gameOptions.teachingDisabled` has no effect on assist ops) and no-leak-into-`snapshot.gameOptions`; existing lockout suite migrated to `hostOptions` (5 fail pre-fix). Deliberately NOT stripping `teachingDisabled` out of `gameOptions` in `executeOp` — that key may be a legitimate game-defined option (the very collision D-01 protects).
