// Drives one full run (all 14 dispatchable slices: 2 enumerators + 1 reconciler each) with
// bounded concurrency. Real `claude -p` dispatches only — no simulation.
...
const BASE = '/private/tmp/claude-501/-Users-jtsmith-BoardSmith/39655717-66f7-4931-ad0d-d86278ea06be/scratchpad/177-20';
...
const ENUM_A_MODEL = 'opus';
const ENUM_B_MODEL = 'haiku';
const RECONCILE_MODEL = 'sonnet';
...
async function processSlice(slice) {
  ...
  try {
    log('dispatching enumerator A (opus)...');
    ...
    log('dispatching enumerator B (haiku)...');
    ...
    // Write plain-text facts-only files for the reconcile dispatch payload.
    ...
    log('dispatching reconciler (sonnet)...');
