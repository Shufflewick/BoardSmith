...
must_haves:
  truths:
    ...
    - "All games use npm (not pnpm) - no pnpm-lock.yaml files exist"
  artifacts:
...
Purpose: Verify that all extracted games can install dependencies, compile TypeScript, and run with `npx boardsmith dev`. Confirm all games use npm (not pnpm) per GAME-11.

Output: All games validated.

Note: Cleanup of extracted games from monorepo (removing from packages/games/, updating vitest.config.ts) is deferred to a future phase. This phase focuses on extraction and validation only.
</objective>
...
<task type="auto">
  <name>Task 3: Verify GAME-11 compliance (npm not pnpm)</name>
  <files>None (validation only)</files>
  <action>
Verify that no extracted game has a pnpm-lock.yaml file, confirming npm is used per GAME-11.

```bash
pnpm_locks=$(find /private/tmp/boardsmith-test-target -name "pnpm-lock.yaml" 2>/dev/null)
if [ -n "$pnpm_locks" ]; then
  echo "ERROR: Found pnpm-lock.yaml files (violates GAME-11):"
  echo "$pnpm_locks"
  exit 1
fi
echo "GAME-11 verified: No pnpm-lock.yaml files found"
```

Also verify package-lock.json exists (from npm install):
```bash
for game in hex checkers cribbage go-fish polyhedral-potions demo-action-panel demo-animation demo-complex-ui floss-bitties; do
  if [ ! -f "/private/tmp/boardsmith-test-target/$game/package-lock.json" ]; then
    echo "WARNING: $game missing package-lock.json"
  fi
done
```
  </action>
  <verify>`find /private/tmp/boardsmith-test-target -name "pnpm-lock.yaml"` returns empty</verify>
  <done>All games use npm (package-lock.json), not pnpm (no pnpm-lock.yaml)</done>
</task>
...
- [ ] No pnpm-lock.yaml files exist (GAME-11)
