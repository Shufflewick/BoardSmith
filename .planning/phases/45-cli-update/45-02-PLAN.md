...
<objective>
Update CLI commands (dev, build, test, pack) to work with standalone game projects that use the `boardsmith` package.

Purpose: Existing commands assume monorepo context with `@boardsmith/*` packages. They need to detect context and behave appropriately for standalone games. This covers CLI-02 through CLI-06.
...
<task type="auto">
  <name>Task 4: Test all commands in both contexts</name>
  <files></files>
  <action>
Build and test all updated commands:
...
2. **Test in standalone game context** (use extracted hex game):
   ```bash
...
3. **Test pack error message in standalone context**:
   ```bash
   cd /private/tmp/boardsmith-test-target/hex
...
# - test: works in both contexts
# - pack: works in monorepo, clear error in standalone
  </verify>
  <done>All CLI commands work correctly in their appropriate contexts</done>
</task>
