...
- It runs four checks: `--changes` (Fallow, changed files), `--duplication` (jscpd, whole repo), `--health-baseline` (baseline drift, #159), `--dupes-baseline` (duplication recorded by content, #232). With no flag, all four run.
- **`.fallow-dupes-baseline.json` is DERIVED and must not be regenerated with `fallow dupes --save-baseline`.** The record is `.fallow-dupes-accepted.json`, keyed by the clone group's own text so an unrelated edit above it cannot invalidate an entry; `boardsmith audit --rekey-dupes` re-addresses only the debt whose content still matches and refuses when any does not. `docs/fallow-gate.md` § "The duplication baseline is keyed by CONTENT" says why.
