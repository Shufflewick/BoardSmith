...
> MCTS-based bot opponents for board games.
...
```typescript
import { createBot, MCTSBot, parseBotLevel, DIFFICULTY_PRESETS } from 'boardsmith/bot';
```
...
- `parseBotLevel()` - Parse difficulty from string (for CLI)

### Classes

- `MCTSBot` - MCTS-based game bot
...
- `BotStrategy` - bot hooks attached to a game definition
...
```typescript
import { createBot, DIFFICULTY_PRESETS, DEFAULT_CONFIG } from 'boardsmith/bot';
...
`BotStrategy` is a set of optional hooks, all of them functions. Attach it to your game definition (or pass it as `createBot`'s last argument).

```typescript
import type { BotStrategy, Objective } from 'boardsmith/bot';
...
const bot = createBot(game, CheckersGame, 'checkers', 2, actionHistory, 'hard', botSeatConfig);
```
...
```typescript
import { parseBotLevel } from 'boardsmith/bot';

parseBotLevel('easy');    // 'easy'
...
```typescript
import { MCTSBot } from 'boardsmith/bot';
...
`BotConfig` in full: `iterations`, `playoutDepth`, `seed`, `async`, `timeout`, `useTranspositionTable`, `parallel`, `useRAVE`, `raveK`, `uctC`, `usePNS`, `pnWeight`, `debug`. See `src/bot/types.ts` for per-field defaults and semantics.
