...
- **The fix (which §3 already endorsed, then S8 contradicted):** keep `$` only for **abstract topology** (`$type`/`$layout`/coords/`$direction`/`$fan`/`$overlap`; `$zone` is OK *only* as a validated enum). Move all cosmetic/value-bearing presentation (`image`/`thumbnail`/`label`/`statBlock`/render-hint) into a **per-UI presentation overlay owned by the `ui` layer** (keyed by className/name, resolved *after* visibility filtering — closing the leak by construction). Each of the N UIs supplies its own overlay; a custom UI ignores it; the engine stays UI-agnostic (preserving headless bot/test/replay).
