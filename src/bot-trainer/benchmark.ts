...
export interface BenchmarkResult {
  /** Overall win rate of trained bot (0-1) */
  winRate: number;
  /** Number of wins */
  wins: number;
  /** Number of losses */
...
export async function benchmarkBot<G extends Game>(
  ...
  // Split games evenly: half with trained bot as player 0, half as player 1
  const halfCount = Math.floor(gameCount / 2);

  // Run games with trained bot as player 0
  ...
  // Run games with trained bot as player 1
  for (let i = 0; i < halfCount; i++) {
    const result = await runBenchmarkGame(
      GameClass,
      gameType,
  ...
  try {
    ...
    while (!flowState.complete && actionCount < maxActions) {
      ...
      if (currentPlayer === trainedPosition) {
        // Trained bot plays with MCTS + objectives
        const bot = createBot(
          runner.game,
          GameClass,
          gameType,
          ...
          mctsIterations,
          objectives.length > 0 ? { objectives: botObjectives } : undefined
        );
