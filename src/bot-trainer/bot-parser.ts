...
/**
 * Parsed objective from an existing bot.ts file
 */
...
/**
 * Parse an existing bot.ts file to extract objectives and metadata
 */
export function parseExistingBot(filePath: string): ParsedBotFile | null {
  ...
  try {
    ...
    return parseBotContent(content);
  } catch (error) {
    console.warn(`Warning: Could not parse existing bot.ts: ${error}`);
    return null;
  }
}

/**
 * Parse bot.ts file content
 */
function parseBotContent(content: string): ParsedBotFile {
  const objectives: ParsedObjective[] = [];
  const metadata: ParsedBotFile['metadata'] = {};
...
export function getCumulativeStats(
  existingMetadata: ParsedBotFile['metadata'] | undefined,
