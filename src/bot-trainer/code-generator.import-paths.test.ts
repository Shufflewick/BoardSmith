...
import { generateBotCode, type CodeGeneratorOptions } from './code-generator.js';
import type { TrainingResult } from './types.js';

/**
 * Regression test for F27: the bot code generator must emit valid `boardsmith`
 * subpath imports, not the nonexistent `@boardsmith/*` scope. Generated bot.ts
...
describe('generateBotCode import paths (F27)', () => {
  ...
  it('emits the published boardsmith package paths', () => {
    const code = generateBotCode(makeResult(), options);
    expect(code).toContain(`import type { Game } from 'boardsmith';`);
    expect(code).toContain(`import type { Objective } from 'boardsmith/bot';`);
  });
