...
export function generateBotCode(
  ...
  // Header
  lines.push(`// AUTO-GENERATED by boardsmith evolve-bot-weights`);
  lines.push(`// Do not edit manually - regenerate with: npx boardsmith evolve-bot-weights`);
  ...
  lines.push(`import type { Objective } from 'boardsmith/bot';`);
  ...
  lines.push(`/**`);
  lines.push(` * bot objectives for ${options.gameClassName.replace('Game', '')}`);
...
function generateCheckerFunction(
  ...
  console.warn(
    `[bot Code Generator] Unknown feature pattern: "${id}". ` +
  ...
  return `() => {
        ...
        // Implement game-specific logic for better bot performance
        return true;
      }`;
}
...
/**
 * Options for updating bot weights
 */
...
/**
 * Update weights in an existing bot.ts file, preserving all other code.
 *
 ...
 *
 * @param originalCode - The original bot.ts file content
