...
describe('initCommand --world — a persistent world project (#168)', () => {
  ...
  async function scaffoldWorld(): Promise<void> {
    parentDir = mkdtempSync(join(tmpdir(), 'bs-init-world-'));
    process.chdir(parentDir);
    await initCommand('tiny-world', { withoutRulebook: true, world: true });
    projectPath = join(parentDir, 'tiny-world');
  }

  const read = (relative: string): string =>
    readFileSync(join(projectPath, relative), 'utf-8');
  ...
  it('writes the four files a world project is', async () => {
    ...
    for (const file of [
      ...
      expect(existsSync(join(projectPath, file)), `${file} is missing`).toBe(true);
    }
  });
  ...
  it('writes no table half — a world has no turn order, flow or action table', async () => {
    ...
    for (const file of [
      ...
      expect(existsSync(join(projectPath, file)), `${file} should not be scaffolded`).toBe(false);
    }
  });
  ...
  it('leaves an ordinary game project untouched', async () => {
    parentDir = mkdtempSync(join(tmpdir(), 'bs-init-table-'));
    process.chdir(parentDir);
    await initCommand('table-game', { withoutRulebook: true });
    projectPath = join(parentDir, 'table-game');
    expect(JSON.parse(read('boardsmith.json')).world).toBeUndefined();
    expect(existsSync(join(projectPath, 'src', 'rules', 'world.ts'))).toBe(false);
