---
name: bs-build-bot
description: Generate a complete bot opponent (all 5 hooks) for a BoardSmith game project. Use as a late sketch chunk after game-end/scoring exists (MCTS needs terminal states); also usable earlier for --bot solo-playtest seat-fill.
---

# BoardSmith bot Generation Instructions

You are helping the user generate custom bot evaluation functions for their BoardSmith game. Your goal is to create a complete bot implementation with all 5 hooks, modeled on production-quality patterns.
...
bot hooks before scoring exists produces objectives with nothing to optimize toward. It can also
be run earlier, standalone, to fill a bot seat for solo `--bot` playtesting during development —
that use does not require the full sketch/chunk state to exist at all, only a compiling
BoardSmith game project.

Under a `/bs-build-game` run this skill is not typed by the designer at all: the run dispatches it
as the bot-opponent chunk's pipeline (`${CLAUDE_SKILL_DIR}/../bs-shared/orchestrate/chunk-dispatch.md`).

## Orchestrated Mode (dispatched, not typed)

A brief that says **"You are running in orchestrated mode"** means there is no designer on the other
end of this context, and the rules of that mode are absolute: **never ask a question, never wait for
approval, and never assume approval was given.** Anything that would be put to the designer —
a difficulty choice, a question about how the opponent should play, an approval of the generated
opponent — stops the work and comes back in the report's `gate`/`questions` fields instead
(`orchestrate/chunk-dispatch.md` "The Return Shape"). Build what the brief's answered-questions
digest already settles, then return; the orchestrator asks, records the answer in the game's
question ledger, and re-dispatches with it. Final message = the report, nothing else.

## The 5 Bot Hooks

A complete bot implementation provides these hooks (all optional, but recommended):
...
Reference: The Hex game bot implementation demonstrates all 5 hooks with production-quality patterns.
...
2. **Check for existing bot.ts**:
   - Look in `src/rules/bot.ts` (or `rules/src/bot.ts` for workspace projects)
   - If exists: "A bot.ts already exists. Should I enhance it, or start fresh?"
...
| Game Type | Examples | Key bot Pattern |
|-----------|----------|----------------|
...
Ask game-type-specific questions to design the bot:
...
## Phase 5: Generate All 5 Bot Hooks

Create `bot.ts` with all 5 hooks following these patterns:

### File Structure

```typescript
// bot evaluation functions for <GameName>
// Generated by /bs-build-bot

import type { Game } from 'boardsmith';
import type { Objective, BotMove, ThreatResponse } from 'boardsmith/bot';
...
**CRITICAL**: bot vs bot benchmarks miss obvious human exploits.

**Manual testing checklist:**
- [ ] Play 3-5 games against the bot
- [ ] Try simple, obvious strategies (straight lines, direct attacks)
- [ ] bot blocks obvious winning threats
- [ ] bot doesn't make random-looking moves late game
- [ ] bot responds to central/key position control
...
After creating the bot, weights can be optimized:

```bash
npx boardsmith evolve-bot-weights --generations 5 --population 20
```

This runs evolutionary optimization on the weights in your bot.ts file, preserving all code structure while tuning weight values for better win rates.
