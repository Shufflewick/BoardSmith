...
/**
 * Restore the exact child order of a space to a captured permutation.
 *
 * Generated only as the inverse of SHUFFLE: shuffling randomly permutes a
 * space's children, so its inverse captures the pre-shuffle order (by element
 * id) and replays it to restore the deck deterministically. This makes SHUFFLE
 * invertible for MCTS incremental undo instead of forcing a full game rebuild.
 *
 * @internal Generated by the inverse-command system; not part of the designer API.
 */
export interface RestoreOrderCommand extends BaseCommand {
  type: 'RESTORE_ORDER';
  /** ID of space whose child order to restore */
  spaceId: number;
  /** Element IDs in the exact order to restore */
  elementIds: number[];
}
