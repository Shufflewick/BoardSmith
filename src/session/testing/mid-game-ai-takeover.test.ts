...
/**
 * BSMITH-03 — a seat converted to AI MID-GAME is actually played by a bot.
 *
 * The mechanism was never the gap. `handleAITurn` takes its seat list as a
 * per-call argument and `runAITurnsInner` re-reads `adapters.aiSeats` on every
 ...
 * without which the engine's own harness froze `aiSeats` at construction and a
...
describe('mid-game AI takeover (BSMITH-03)', () => {
  // ── 1. The bot MOVES ───────────────────────────────────────────────────────
  ...
  it('case 1 — a seat converted mid-game is played by a bot, with no further human input', async () => {
    ...
    session.makeSeatAI(2);
    const res = await session.send(2, { type: 'convertSeatToAI', seat: 2 });

    expect(res.success, `convertSeatToAI failed: ${res.error ?? ''}`).toBe(true);
  ...
  it('case 2 — the conversion op ALONE drives the pump; flipping the roster without it parks the table', async () => {
    ...
    // reports seat 2 as AI and the pump would honour it — but nothing wakes it.
    session.makeSeatAI(2);
    expect(lastBoundary(session)).toEqual(parked);
    expect(await actionHistory(session)).toEqual([{ name: 'commit', player: 1 }]);

    // The op is the wake. No `host.runAITurns()` anywhere in this test: if the
    // test had to call it, gap B would still be open.
    await session.send(2, { type: 'convertSeatToAI', seat: 2 });
  ...
  it('case 3 — a converted seat and a seat that started as AI produce the SAME observable play', async () => {
    // Started-as-AI. `start()` does not pump, so seat 1 moves first in both.
    const startedAsAI = createHeadlessSession(simultaneousRoundsFixtureDefinition, twoSeats, [
      { seat: 2 },
    ]);
    await startedAsAI.start();
    await commit(startedAsAI, 1);
    ...
    converted.makeSeatAI(2);
    await converted.send(2, { type: 'convertSeatToAI', seat: 2 });

    // Same boundary identity and same due seats: the converted seat is at the
    // exact flow position the pre-configured one reached.
    expect(lastBoundary(converted)).toEqual(lastBoundary(startedAsAI));
    ...
    expect(await actionHistory(converted)).toEqual(await actionHistory(startedAsAI));
  });

  // ── 4. Loud on a conversion the roster does not back ───────────────────────

  it('case 4 — converting a seat the roster does not report as AI is refused, loudly and actionably', async () => {
    const session = createHeadlessSession(simultaneousRoundsFixtureDefinition, twoSeats);
    await session.start();
    await commit(session, 1);
    const before = lastBoundary(session);

    // NOTE: no `makeSeatAI(2)`. This is the silent failure being closed — an op
  ...
  it('case 5 — converting an already-converted seat succeeds and does not move the game again', async () => {
    ...
    session.makeSeatAI(2);
    const first = await session.send(2, { type: 'convertSeatToAI', seat: 2 });
    ...
    // is already a bot.
    const second = await session.send(2, { type: 'convertSeatToAI', seat: 2 });
  ...
  it('case 7 — a conversion-started pump is still capped at MAX_AI_MOVES', async () => {
    ...
    // MAX_AI_MOVES bounds. The exact cap AND the exact log line are what prove
    // the conversion went through `runAITurnsInner` and not a clone of it.
    const base: OpResult = {
      success: true,
      snapshot: {},
      pendingState: null,
    ...
    const res = await host.handleOp(2, { type: 'convertSeatToAI', seat: 2 });
