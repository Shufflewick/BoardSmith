...
export function readOnlyProjection<T>(value: T): T {
  ...
  const projection = new Proxy(subject, {
    get(target, property, receiver) {
      ...
      if (takesProjectionAsReceiver(target)) {
        return function (...args: unknown[]) {
          return readOnlyProjection(Reflect.apply(call, receiver, args));
        };
      }
      if (BUILTIN_MUTATORS.has(property as string)) refuseWrite(property);
      return (...args: unknown[]) => readOnlyProjection(Reflect.apply(call, target, args));
    },
